...
func (c *Counter) With(labelValues ...string) xstat.Counter {
	return &Counter{
		Name: c.Name,
...
func (c *Counter) Add(delta float64) {
	for {
		var (
			...
			new1 = math.Float64frombits(old) + delta
			new2 = math.Float64bits(new1)
		)
		if atomic.CompareAndSwapUint64(&c.bits, old, new2) {
			break
		}
	}
}
...
// This is useful for xmetric backends whose counter aggregations expect deltas,
// like Graphite.
...
func (g *Gauge) With(labelValues ...string) xstat.Gauge {
	return &Gauge{
		Name: g.Name,
...
func (g *Gauge) Add(delta float64) {
	for {
		...
		if atomic.CompareAndSwapUint64(&g.bits, old, new2) {
			break
		}
	}
}
...
func (h *Histogram) With(labelValues ...string) xstat.Histogram {
	return &Histogram{
		Name: h.Name,
...
// With implements Histogram.
func (h *SimpleHistogram) With(labelValues ...string) xstat.Histogram {
	return &SimpleHistogram{
		lvs: h.lvs.With(labelValues...),
		avg: h.avg,
		n:   h.n,
